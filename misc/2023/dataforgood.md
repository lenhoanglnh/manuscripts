...
l'un de ses algorithmes génératifs, est composée de conversations sur les réseaux sociaux[^palm],
il semble y avoir de quoi fortement douter de la conformité des algorithmes génératifs avec le RGPD.
D'autant que, à maintes reprises, il a été montré que ces algorithmes mémorisent ces données d'entraînement[^memorization]. Voilà qui a conduit Samsung à interdire l'utilisation des algorithmes génératifs à une partie de ses employés[^samsung_interdiction], après que plusieurs d'entre eux ont révélés des secrets industriels à ChatGPT[^samsung_erreur]. Même Microsoft appelle ses employés à faire attention à leur utilisation de ChatGPT[^microsoft_chatgpt], tandis que Google demande aux siens de ne pas utilisés le code généré par Google Bard[^google_bard].
...
ainsi que des risques pour les secrets industriels et la cybersécurité,
...
Et si la loi déjà en vigueur ne s'applique pas, peut-on faire confiance à l'application des lois futures, 
comme l'AI Act et le Cyber Resilience Act, parmi d'autres ?
En particulier, les appels aux régulations futures ne sont-ils pas une façon de dévier l'attention des lois déjà existantes, et déjà probablement violées ?
...
simplement en rendant extrêmement opaques leur conception et leur déploiement.
Tant que des moyens conséquents ne seront pas mis en oeuvre pour faire appliquer les lois existantes,
...
dans cet espace que des milliards d'entre nous consultons quotidiennement,
...
comme cela est le cas dans les industries matûres comme l'aviation, la pharmaceutique et l'agroalimentaire.

## Dépendance et sobriété numérique

La régulation des algorithmes est cependant souvent présentée comme un épouvantail,
qui freinera l'émergence de grandes entreprises numériques européennes.
Si cette critique a clairement ses propres limites 
(une entreprise européenne est-elle vraiment plus digne de confiance, si elle est par conception hors de tout contrôle démocratique ?),
elle soulève toutefois un problème plus établi et plus concernant 
sur la dépendance numérique.
En particulier, il est urgent de considérer que les fournisseurs de services informatiques 
peuvent un jour devenir des menaces dangereuses.
Or, de nombreuses entreprises européennes sont déjà devenues extrêmement dépendantes 
du service informatique de Google, Microsoft ou Amazon, parmi d'autres.
Si ces entreprises cessent leur service, 
...
À l'heure où des solutions numériques émergentes viennent de pays autoritaires,
à l'instar du modèle de langue Falcon conçu par l'entreprise Émirati *Technology Institute Innovation* et néanmoins promue par Hugging Face[^hugging_face_falcon],
la dépendance numérique correspond à de sérieux enjeux de géopolitique
...
parce que des chercheurs d'Alibaba ont rapporté la vulnérabilité log4j à Oracle,
...
En fait, de manière totalement opposée à l'approche généralistes des algorithmes génératifs,
les experts en cybersécurité appellent souvent à réduire les fonctionnalités des algorithmes,
afin de réduire la *surface d'attaque*.
C'est ce qu'on pourrait appeler le principe de *sobriété numérique*.
Typiquement, intégrer une domotique connectée, c'est augmenter notre dépendance numérique
en les nombreux hardwares et softwares impliquées dans cette domotique connectée.
De la même manière, chaque nouveau object connecté implique de nouvelles vulnérabilités potentielles,
tout comme chaque importation de packages python[^calicarpa] ou chaque nouvel outil de la suite Google.
La meilleure cyberdéfense, c'est d'éviter les *bloatwares*,
...
Tout le contraire des algorithmes génératifs, dans la plupart des applications.

[^calicarpa]: https://calicarpa.com

Pour augmenter la robustesse des entreprises, 
il est urgent d'imposer des régulations qui favoriseront la sobriété numérique de toutes les entreprises,
surtout celles qui ne sont pas par nature des entreprises technologiques.
Pour cela, la régulation est en fait un atout.
Or, de façon très préoccupante, la Commission Européenne semble avoir cédé au lobby d'OpenAI,
une entreprise américaine pourtant largement reconnue 
pour ses très faibles standards de transparence et de sécurité.
En tout cas, elle a amendé l'AI Act pour tolérer les algorithmes génératifs avec un moindre contrôle
...
les capacités de l'industrie de la désinformation à automatiser et crédibiliser les créations et les activités des faux comptes
...
le développeur de Falcon,
est aussi le fondateur et directeur de DarkMatters, 
une autre entreprise Émirati spécialisée dans les cyber-attaques et sous enquête par le FBI[^darkmatters].
Sachant à quel point les cyberdéfenses sont à la traîne[^perlroth],
...
sur la sécurité de ces systèmes, l'indépendance vis-à-vis d'eux
...
Par ailleurs, les intelligences artificielles les plus lucratives et les plus influentes d'aujourd'hui ne sont pas des algorithmes génératifs.
...
Or il y a une inattention alarmante à la conception, à la sécurité et aux conséquences socio-économiques de ces IA,
...
ce sont sans doute ces intelligences artificielles qu'il faudrait urgemment beaucoup plus strictement encadrer.
